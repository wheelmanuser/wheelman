...
function LayoutInner({ children }: { children: React.ReactNode }) {
  ...
  const emailFallback = user?.email?.split("@")[0] ?? "Driver";
  const displayName = settings.display_name || emailFallback;
          ...
          >
            {settings.avatar_url ? (
              ...
              <img
                ...
                alt="Avatar"
                className="h-7 w-7 rounded-sm border border-wm-accent/40 object-cover"
              />
            ) : (
...
export function DashboardLayoutClient({
  ...
  return (
    <UserSettingsProvider initial={initialSettings}>
