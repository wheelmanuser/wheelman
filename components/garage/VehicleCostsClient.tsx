...
export function VehicleCostsClient({
    ...
    supabase
      ...
      .then(({ data, error: err }) => {
        if (err) {
          ...
          setEntries((data ?? []) as CostEntry[]);
        }
        setLoading(false);
      });
  }, [vehicleId]);

  if (loading) return <p className="text-sm text-wm-text2">Loading costs...</p>;
  if (error) return <p className="text-sm text-wm-red">{error}</p>;

  const now = new Date();
  const currentYear = now.getFullYear();
  const odometer = Number(odometerMiles ?? 0);
  ...
  for (const e of entries) {
    ...
    monthlyByYear[year] ??= MONTHS.map((label, idx) => ({ month: idx + 1, label, value: 0 }));
    monthlyByYear[year][month - 1].value += Number(e.total_cost ?? 0);
  }
  if (!monthlyByYear[currentYear]) {
    monthlyByYear[currentYear] = MONTHS.map((label, idx) => ({ month: idx + 1, label, value: 0 }));
  }
  const years = Object.keys(monthlyByYear).map(Number).sort((a, b) => b - a);
